...
export function useChunkedUpload() {
  ...
  const pauseUpload = useCallback(async (uploadId) => {
    try {
      ...
      const res = await api.post(`/files/chunked/pause/${uploadId}`)
      ...
      setUploads(prev => ({
        ...
        [uploadId]: {
          ...
          pauseCount: res.data.pauseCount
        }
      }))
  ...
  const resumeUpload = useCallback(async (uploadId) => {
    try {
      // Create new AbortController for resumed upload
      abortControllers.current[uploadId] = new AbortController()
      ...
      const res = await api.post(`/files/chunked/resume/${uploadId}`)
  ...
  const continueFileUpload = useCallback(async (uploadId, file, onProgress, onUploadIdReady) => {
    try {
      // Get current upload status
      const statusRes = await api.get(`/files/chunked/status/${uploadId}`)
      const { uploadedChunks, totalChunks, fileSize, chunkSize } = statusRes.data
      
      // Get upload session details
      const uploadSession = uploads[uploadId]
      if (!uploadSession) {
        throw new Error('Upload session not found in local state')
      }

      // Create AbortController if needed
      if (!abortControllers.current[uploadId]) {
        abortControllers.current[uploadId] = new AbortController()
      }

      if (onProgress) onProgress(Math.round((uploadedChunks.length / totalChunks) * 95))

      let completedChunks = uploadedChunks.length
      const chunkSizes = {}
      const inFlightProgress = {}
      
      // Initialize chunk sizes for progress calculation
      ...
      const reportProgressFromBytes = () => {
        ...
        for (const key in chunkSizes) {
          const size = chunkSizes[key]
          const percent = inFlightProgress[key] ?? (uploadedChunks.includes(parseInt(key)) ? 100 : 0)
          uploadedBytes += (percent / 100) * size
        }
        const progress = Math.min(95, Math.round((uploadedBytes / fileSize) * 95))
        if (onProgress) {
          onProgress(progress)
        }
      }
      ...
      for (let i = 1; i <= totalChunks; i += parallel) {
        ...
        for (let j = 0; j < parallel && i + j <= totalChunks; j++) {
          ...
          if (uploadedChunks.includes(chunkNum)) {
            inFlightProgress[chunkNum] = 100
            continue
          }
